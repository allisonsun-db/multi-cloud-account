...
export function LocationPicker({ value, onChange, cloudRegions }: {
  ...
  const label = value === "all" ? "Select region"
  ...
  return (
    <Popover open={open} onOpenChange={(o) => { setOpen(o); if (!o) setActiveCloud(null) }}>
      ...
      <PopoverContent className="w-52 p-1" align="start">
        {activeCloud === null ? (
          <>
            <button onClick={() => select("all")} className="flex w-full items-center rounded px-2 py-1.5 text-sm text-muted-foreground hover:bg-muted">
              Select cloud first
            </button>
            {(["AWS", "Azure", "GCP"] as const).map((c) => (
              <button key={c} onClick={() => setActiveCloud(c)} className={cn("flex w-full items-center justify-between rounded px-2 py-1.5 text-sm hover:bg-muted", value === c && "text-primary")}>
          ...
          <>
            <button onClick={() => setActiveCloud(null)} className="flex w-full items-center gap-1 rounded px-2 py-1.5 text-sm text-muted-foreground hover:bg-muted">
              ...
              <img src={CLOUD_LOGO[activeCloud]} alt={activeCloud} width={14} height={14} className={cn("object-contain", activeCloud === "AWS" && "dark:[filter:brightness(0)_invert(1)]")} />
              {activeCloud}
            </button>
            <div className="my-1 border-t border-border" />
            {(cloudRegions[activeCloud] ?? []).map((r) => (
              <button key={r} onClick={() => select(`${activeCloud}:${r}`)} className={cn("flex w-full items-center rounded px-2 py-1.5 text-sm hover:bg-muted", value === `${activeCloud}:${r}` && "text-primary")}>
                {r}
              </button>
            ))}
          </>
